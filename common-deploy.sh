...
echo -e "${CYAN}Starting TrueAmino deployment with CommonJS...${NC}"
...
# Make sure package.json doesn't have "type": "module"
if grep -q '"type":\s*"module"' package.json; then
    echo -e "${YELLOW}Removing ES module type from package.json...${NC}"
    sed -i 's/"type":\s*"module",\?//g' package.json
    echo -e "${GREEN}Fixed package.json to use CommonJS${NC}"
fi
...
# Start the CommonJS static server that immediately binds to port 5000
echo -e "${GREEN}Starting the CommonJS static server to bind port 5000 immediately...${NC}"
exec node static-commonjs.js