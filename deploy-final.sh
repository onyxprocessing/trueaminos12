...
echo -e "${CYAN}Starting TrueAmino deployment...${NC}"
...
if [ -d "$REPO_NAME" ]; then
    echo -e "${YELLOW}$REPO_NAME directory already exists. Continuing with existing codebase.${NC}"
else
...
# Go back to root directory and start the server
cd ..
echo -e "${CYAN}Starting server...${NC}"
exec node server-starter.js